-- Corregir función RPC para manejar correctamente el enum eye_side
DROP FUNCTION IF EXISTS get_clinical_research_data;
...
BEGIN
  ...
  exam_data AS (
    SELECT
      be.encounter_id,
      -- OD (cast side to text for comparison)
      ...
      -- OS (cast side to text for comparison)
      MAX(CASE WHEN ee.side::text = 'OS' THEN ee.av_sc END) as av_sc_os,
      MAX(CASE WHEN ee.side::text = 'OS' THEN ee.av_cc END) as av_cc_os,
      MAX(CASE WHEN ee.side::text = 'OS' THEN ee.ref_subj_sphere END) as ref_subj_sphere_os,
      MAX(CASE WHEN ee.side::text = 'OS' THEN ee.ref_subj_cyl END) as ref_subj_cyl_os,
      MAX(CASE WHEN ee.side::text = 'OS' THEN ee.ref_subj_axis END) as ref_subj_axis_os,
      MAX(CASE WHEN ee.side::text = 'OS' THEN ee.ref_subj_av END) as ref_subj_av_os,
      MAX(CASE WHEN ee.side::text = 'OS' THEN ee.rx_sphere END) as rx_sphere_os,
      MAX(CASE WHEN ee.side::text = 'OS' THEN ee.rx_cyl END) as rx_cyl_os,
      MAX(CASE WHEN ee.side::text = 'OS' THEN ee.rx_axis END) as rx_axis_os,
      MAX(CASE WHEN ee.side::text = 'OS' THEN ee.rx_add END) as rx_add_os,
      MAX(CASE WHEN ee.side::text = 'OS' THEN ee.prescription_notes END) as prescription_notes_os,
      MAX(CASE WHEN ee.side::text = 'OS' THEN ee.slit_lamp END) as slit_lamp_os,
      MAX(CASE WHEN ee.side::text = 'OS' THEN ee.fundus END) as fundus_os,
      MAX(CASE WHEN ee.side::text = 'OS' THEN ee.iop END) as pio_exam_os,
      MAX(CASE WHEN ee.side::text = 'OS' THEN ee.plan END) as plan_os
    FROM base_encounters be
