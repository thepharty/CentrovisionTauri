-- Actualizar políticas RLS para incluir 'contabilidad'

-- service_prices
DROP POLICY IF EXISTS "Admin y caja pueden gestionar precios de servicios" ON public.service_prices;
CREATE POLICY "Admin, caja y contabilidad pueden gestionar precios de servicios"
...
CREATE POLICY "Admin, caja y contabilidad pueden gestionar facturas"
ON public.invoices
FOR ALL
USING (
  public.has_role(auth.uid(), 'admin'::app_role) OR 
...
CREATE POLICY "Admin, caja y contabilidad pueden gestionar items de factura"
...
CREATE POLICY "Admin, caja y contabilidad pueden gestionar pagos"
ON public.payments
FOR ALL
USING (
  public.has_role(auth.uid(), 'admin'::app_role) OR 
...
CREATE POLICY "Admin, caja y contabilidad pueden gestionar inventario"
...
CREATE POLICY "Admin, caja y contabilidad pueden gestionar lotes"
...
CREATE POLICY "Admin, caja y contabilidad pueden gestionar movimientos de inventario"
...
CREATE POLICY "Admin, caja y contabilidad pueden crear cierres"
...
CREATE POLICY "Admin, caja y contabilidad pueden ver cierres"
ON public.cash_closures
FOR SELECT
TO authenticated
USING (
...
CREATE POLICY "Admin, caja y contabilidad pueden gestionar proveedores"
ON public.suppliers
FOR ALL
USING (
  has_role(auth.uid(), 'admin'::app_role) OR 
