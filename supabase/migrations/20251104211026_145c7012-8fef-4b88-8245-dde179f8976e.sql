-- Update get_clinical_research_data to support 'all' search field type
    ...
    WHERE e.date >= start_date
      ...
      AND (
        ...
        CASE search_field_type
          WHEN 'all' THEN (
            e.summary ILIKE '%' || diagnosis_filter || '%' OR
            e.plan_tratamiento ILIKE '%' || diagnosis_filter || '%' OR
            e.cirugias ILIKE '%' || diagnosis_filter || '%' OR
            e.estudios ILIKE '%' || diagnosis_filter || '%' OR
            e.motivo_consulta ILIKE '%' || diagnosis_filter || '%' OR
            EXISTS (
...
-- Update get_clinical_research_data_by_patient to support 'all' search field type
