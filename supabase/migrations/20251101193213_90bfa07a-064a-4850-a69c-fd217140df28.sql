-- Función RPC para extraer datos de investigación clínica
CREATE OR REPLACE FUNCTION get_clinical_research_data(
  start_date timestamptz,
  end_date timestamptz,
  doctor_filter uuid DEFAULT NULL,
  diagnosis_filter text DEFAULT NULL,
  surgery_type_filter text DEFAULT NULL,
  encounter_type_filter encounter_type DEFAULT NULL,
...
RETURNS TABLE (
  ...
  -- Pre-Consulta (Objetivo)
  autorefractor text,
  lensometry text,
  pio_od_preconsult numeric,
  pio_os_preconsult numeric,
  ...
  ref_subj_sphere_od numeric,
  ref_subj_cyl_od numeric,
  ref_subj_axis_od integer,
  ref_subj_av_od text,
  
  -- Refracción Subjetiva OS
  ref_subj_sphere_os numeric,
  ref_subj_cyl_os numeric,
  ref_subj_axis_os integer,
  ref_subj_av_os text,
  
  -- Receta OD
  rx_sphere_od numeric,
  rx_cyl_od numeric,
  rx_axis_od integer,
  rx_add_od numeric,
  prescription_notes_od text,
  
  -- Receta OS
  rx_sphere_os numeric,
  rx_cyl_os numeric,
  rx_axis_os integer,
  rx_add_os numeric,
  prescription_notes_os text,
  ...
  cirugias_recomendadas text,
  estudios_recomendados text,
  proxima_cita text,
...
BEGIN
  ...
  WITH base_encounters AS (
    ...
    WHERE e.date >= start_date
      ...
      AND (encounter_type_filter IS NULL OR e.type = encounter_type_filter)
      AND (diagnosis_filter IS NULL OR e.summary ILIKE '%' || diagnosis_filter || '%')
  ),
  patient_data AS (
    SELECT
      be.encounter_id,
  ...
  exam_data AS (
    SELECT
      be.encounter_id,
      -- OD
      MAX(CASE WHEN ee.side = 'OD' THEN ee.av_sc END) as av_sc_od,
      MAX(CASE WHEN ee.side = 'OD' THEN ee.av_cc END) as av_cc_od,
      MAX(CASE WHEN ee.side = 'OD' THEN ee.ref_subj_sphere END) as ref_subj_sphere_od,
      MAX(CASE WHEN ee.side = 'OD' THEN ee.ref_subj_cyl END) as ref_subj_cyl_od,
      MAX(CASE WHEN ee.side = 'OD' THEN ee.ref_subj_axis END) as ref_subj_axis_od,
      MAX(CASE WHEN ee.side = 'OD' THEN ee.ref_subj_av END) as ref_subj_av_od,
      MAX(CASE WHEN ee.side = 'OD' THEN ee.rx_sphere END) as rx_sphere_od,
      MAX(CASE WHEN ee.side = 'OD' THEN ee.rx_cyl END) as rx_cyl_od,
      MAX(CASE WHEN ee.side = 'OD' THEN ee.rx_axis END) as rx_axis_od,
      MAX(CASE WHEN ee.side = 'OD' THEN ee.rx_add END) as rx_add_od,
      MAX(CASE WHEN ee.side = 'OD' THEN ee.prescription_notes END) as prescription_notes_od,
      MAX(CASE WHEN ee.side = 'OD' THEN ee.slit_lamp END) as slit_lamp_od,
      MAX(CASE WHEN ee.side = 'OD' THEN ee.fundus END) as fundus_od,
      MAX(CASE WHEN ee.side = 'OD' THEN ee.iop END) as pio_exam_od,
      MAX(CASE WHEN ee.side = 'OD' THEN ee.plan END) as plan_od,
      -- OS
      MAX(CASE WHEN ee.side = 'OS' THEN ee.av_sc END) as av_sc_os,
      MAX(CASE WHEN ee.side = 'OS' THEN ee.av_cc END) as av_cc_os,
      MAX(CASE WHEN ee.side = 'OS' THEN ee.ref_subj_sphere END) as ref_subj_sphere_os,
      MAX(CASE WHEN ee.side = 'OS' THEN ee.ref_subj_cyl END) as ref_subj_cyl_os,
      MAX(CASE WHEN ee.side = 'OS' THEN ee.ref_subj_axis END) as ref_subj_axis_os,
      MAX(CASE WHEN ee.side = 'OS' THEN ee.ref_subj_av END) as ref_subj_av_os,
      MAX(CASE WHEN ee.side = 'OS' THEN ee.rx_sphere END) as rx_sphere_os,
      MAX(CASE WHEN ee.side = 'OS' THEN ee.rx_cyl END) as rx_cyl_os,
      MAX(CASE WHEN ee.side = 'OS' THEN ee.rx_axis END) as rx_axis_os,
      MAX(CASE WHEN ee.side = 'OS' THEN ee.rx_add END) as rx_add_os,
      MAX(CASE WHEN ee.side = 'OS' THEN ee.prescription_notes END) as prescription_notes_os,
      MAX(CASE WHEN ee.side = 'OS' THEN ee.slit_lamp END) as slit_lamp_os,
      MAX(CASE WHEN ee.side = 'OS' THEN ee.fundus END) as fundus_os,
      MAX(CASE WHEN ee.side = 'OS' THEN ee.iop END) as pio_exam_os,
      MAX(CASE WHEN ee.side = 'OS' THEN ee.plan END) as plan_os
    FROM base_encounters be
    ...
    SELECT DISTINCT
      ...
      EXISTS(
        ...
        WHERE e2.patient_id = be.patient_id
          ...
          AND e2.type IN ('reconsulta', 'consulta')
